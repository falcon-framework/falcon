  ...
  handlers
      ...
      Effect.gen(function* () {
        ...
        const app = yield* svc.getApp(appId).pipe(
          ...
          Effect.catchTag("DatabaseError", () =>
            Effect.fail(new ApiNotFoundError({ message: germanMessages.appNotFound(appId) })),
          ),
        );
        void app;
    ...
    .handle("approveInstallationRequest", ({ path: { requestId } }) =>
      Effect.gen(function* () {
        const principal = yield* PrincipalTag;
        const svc = yield* InstallationService;
        const conn = yield* svc.approve(principal, requestId).pipe(
          Effect.catchTag("ForbiddenError", (e) =>
            Effect.fail(new ApiForbiddenError({ message: e.reason })),
          ),
          Effect.catchTag("NotFoundError", (e) =>
    ...
    .handle("getConnection", ({ path: { connectionId } }) =>
      Effect.gen(function* () {
        const principal = yield* PrincipalTag;
        const svc = yield* ConnectionService;
        const detail = yield* svc.getDetail(principal, connectionId).pipe(
          Effect.catchTag("NotFoundError", (e) =>
            Effect.fail(
              new ApiNotFoundError({ message: `${e.resource} '${e.id}' nicht gefunden.` }),
            ),
    ...
    .handle("revokeConnection", ({ path: { connectionId } }) =>
      Effect.gen(function* () {
        const principal = yield* PrincipalTag;
        const svc = yield* ConnectionService;
        const conn = yield* svc.revoke(principal, connectionId).pipe(
          Effect.catchTag("ForbiddenError", (e) =>
            Effect.fail(new ApiForbiddenError({ message: e.reason })),
          ),
          Effect.catchTag("NotFoundError", (e) =>
    ...
    .handle("pauseConnection", ({ path: { connectionId } }) =>
      Effect.gen(function* () {
        const principal = yield* PrincipalTag;
        const svc = yield* ConnectionService;
        const conn = yield* svc.pause(principal, connectionId).pipe(
          Effect.catchTag("ForbiddenError", (e) =>
            Effect.fail(new ApiForbiddenError({ message: e.reason })),
          ),
          Effect.catchTag("NotFoundError", (e) =>
    ...
    .handle("triggerSync", ({ path: { connectionId } }) =>
      Effect.gen(function* () {
        ...
        const job = yield* svc.triggerSync(principal, connectionId).pipe(
          Effect.catchTag("ForbiddenError", (e) =>
            Effect.fail(new ApiForbiddenError({ message: e.reason })),
          ),
          Effect.catchTag("NotFoundError", (e) =>
