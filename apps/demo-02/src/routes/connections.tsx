...
function ConnectionsPage() {
  ...
  function renderRows(rows: FalconConnectConnectionDisplay[]) {
    if (rows.length === 0) {
      return (
        <p className="text-sm text-[var(--sea-ink-soft)]">None yet.</p>
      )
    }
    return (
    ...
    <main className="page-wrap px-4 py-12">
      <div className="island-shell max-w-2xl rounded-2xl p-8">
        ...
        <p className="mb-8 text-sm text-[var(--sea-ink-soft)]">
          Apps linked to your organization via Falcon Connect. Names come from
          the platform app directory (<code className="rounded bg-[var(--chip-bg)] px-1 text-xs">GET /v1/apps</code>).
        </p>
        ...
        {!isLoaded ? (
          ...
          <div className="space-y-10">
            ...
            <section>
              ...
              <p className="mb-4 text-xs text-[var(--sea-ink-soft)]">
                If this app initiated an install to another target, it appears
                here.
              </p>
              {renderRows(outgoing)}
            </section>
