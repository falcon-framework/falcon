...
function DashboardPage() {
  ...
  async function startConnect() {
    ...
    try {
      ...
      const requestedScopes = caps.length > 0 ? caps.map((c) => c.scopeKey) : ["demo.read"];
      ...
      setError(e instanceof Error ? e.message : "Could not start connection flow");
    } finally {
      setBusy(false);
    }
  }
    ...
    <main className="page-wrap px-4 py-12">
      <div className="island-shell max-w-2xl rounded-2xl p-8">
        ...
        <p className="mb-6 text-sm text-[var(--sea-ink-soft)]">
          This uses Falcon Auth for sign-in and Falcon Connect to create an installation request.
          You will be sent to the target app on port{" "}
          <code className="rounded bg-[var(--chip-bg)] px-1">3011</code> to approve the connection,
          then returned here.
        </p>
        {error ? (
