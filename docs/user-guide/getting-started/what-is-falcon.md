...
1. **Falcon Auth** — A centralized identity service (similar in *shape* to Clerk or Auth0’s hosted experience). Your applications send users to Falcon to sign in or sign up; after authentication, users return to your app with a session that is anchored on the auth service.
...
| Term | Meaning |
| ---- | ------- |
| **Auth server** | HTTP service exposing Better Auth–compatible routes (for example `/api/auth/*`) and Falcon’s hosted sign-in UI (`/hosted/sign-in`, `/hosted/sign-up`). |
...
- **Auth** answers: *Who is this user?* Sessions and users live in the auth database; cookies for the session are set on the auth server’s origin (with appropriate cross-site settings for embedded API calls from your app’s origin).
- **Connect** answers: *May this organization’s installation allow app A to act on app B under scope S?* It does not replace Auth; it relies on knowing the **user** and **organization** for each API call.
